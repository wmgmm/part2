...
**The arc, said out loud at the close:** the day mirrors the plan's own change framework: Discover (01), Understand (02, 03), Act (04, 05), Lead (06). Scattered experiments, then shared learning, then embedding AI-enabled ways of working in team processes. Mission 6's last step and the Useful Prompts strip are the "embed it" move.

## Session plans

**90 minutes:** all six missions (01-06), core paths only, choice paths as take-home.
**60 minutes:** missions 01, 04 and 05 (research, infographic duel, post-mortem-and-chain); swap 05 for 03 (the data table audit) in a data-curious room.
Either way: everyone fires Mission 01's Deep Research run in the first five minutes, then works around it (a good wait-filler: the Useful Prompts strip's "What can these tools do for my job?" prompt, run with their real role).
Close by pointing everyone at the **Useful Prompts strip below the mission grid**: the mission prompts stripped generic. It is the take-home.
...
**Closer (recommended):** while people finish Mission 6, generate an Audio Overview of the day's own briefing pack in a Gemini Notebook. Two synthetic hosts earnestly discussing whether the AI statement survives the Demand First test doubles as the wrap-up.
...
3. On a STANDARD staff account (not yours): check Gemini Notebook Studio shows **Slide Deck** generation (education-tier gated; Mission 6 has a fallback line if absent), and that Deep Research is visible in Gemini's tool picker.
...
### The Useful Prompts strip (not a mission)
Below the mission grid; opens the library page (#/prompts). Fourteen generic rows, including "Chain a second opinion", "Copy the style of a document you like" and the restored spreadsheet pair. Point at "What can these tools do for my job?" first; it is the orientation prompt.
