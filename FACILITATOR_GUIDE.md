...
**90 minutes:** all six missions, core paths only, choice paths ("CHOOSE YOUR NEXT MOVE") as take-home.
**60 minutes:** missions 01, 03 and 06 (research → customers → app). Say so out loud at the start.
Either way: everyone fires Mission 01's Deep Research run in the first five minutes, then works around it.
...
1. Open the site; log in with a test name; open each mission; click one DOWNLOAD per mission.
