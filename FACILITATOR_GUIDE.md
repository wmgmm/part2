...
**The box truncates silently at 5,000 characters**, prompt included, and gives no warning: it simply stops mid-word and generates from the fragment. The skill is 4,730, so it fits with about 190 characters spare. If anyone edits it before a session, check the length or the deck will be built from half a rule set.
