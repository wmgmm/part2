...
Core step time is 72 minutes, bonus excluded, so nobody runs all six.
...
| `backup` | `{text, attach, attachLabel}`: a BACKUP card under the prompt for the fallback route, optionally with its own attach strip |
