...
| `backup` | `{label, text, examples, after, attach, attachLabel, attachExtra}`: a card under the prompt for the other route, label defaults to BACKUP. `examples` is a list of `[from, to]` swaps, `after` a closing line, and the strip works as the step's own |
