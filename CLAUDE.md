...
**Artifacts** are served from `public/placeholders/` and downloaded via a synthetic `<a download>` (same-origin only). All eight venture files are authored and regenerate deterministically via `python3 tools/make_artifacts.py`.
