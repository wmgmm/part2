...
- Each mission page names the tool, links to it, shows a visual workflow, downloadable documents, copyable prompts, and optional "CHOOSE YOUR NEXT MOVE" paths. Mission 05 teaches chaining: a Gemini post-mortem adjudicated in Copilot.
- Every prompt uses the bracket convention: `[square brackets]` mark what to swap for your own work; the default inside them means prompts run verbatim. The Useful Prompts strip below the grid collects the generic templates plus meta-prompting. Mission 03 turns the plan into a data table with Notebook Data Tables and audits it with code.
